...
TRACKING_COLUMN_NAMES_V2 = ['type', 'truncated', 'occluded', 'alpha', 'bbox_left', 'bbox_top','bbox_right', 'bbox_bottom', 'height', 'width', 'length', 'pos_x', 'pos_y', 'pos_z', 'rot_y', 'undefined']
...
def read_tracking_v2(path, frame):
    df = pd.read_csv(osp.join(path, frame), header=None, sep=' ')
    df.columns = TRACKING_COLUMN_NAMES_V2
    df = df[df.type.isin(['Car', 'Pedestrian', 'Cyclist'])]
    return df